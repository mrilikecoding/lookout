...
async fn show_text_pushes_a_card() -> anyhow::Result<()> {
    ...
    let default_session: Arc<dyn Fn() -> SessionId + Send + Sync> =
        Arc::new(|| "integration-test".to_string());
...
async fn show_text_with_invalid_format_returns_error() -> anyhow::Result<()> {
    ...
    let default_session: Arc<dyn Fn() -> SessionId + Send + Sync> =
        Arc::new(|| "integration-test-err".to_string());
    let server = McpServer::bind(0, cmd_tx, default_session).await?;
    ...
    // tools/call with a bogus format
    let resp = client
        .post(&base_url)
        .header("Content-Type", "application/json")
        .header("Accept", "application/json, text/event-stream")
        ...
        .json(&serde_json::json!({
            ...
            "params": {
                "name": "show_text",
                "arguments": { "content": "x", "format": "html" }
            }
        }))
        .send()
        .await?;
    ...
    assert!(
        has_error,
        "expected an error response for invalid format, got: {parsed}"
    );
