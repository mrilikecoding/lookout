...
async fn show_log_text_splits_into_entries() -> anyhow::Result<()> {
    ...
    let default_session: Arc<dyn Fn() -> SessionId + Send + Sync> =
        Arc::new(|| "integration-test-log".to_string());
    ...
    // ── Step 3: tools/call show_log with text ────────────────────────────────
...
async fn show_log_with_neither_input_returns_error() -> anyhow::Result<()> {
    ...
    let default_session: Arc<dyn Fn() -> SessionId + Send + Sync> =
        Arc::new(|| "integration-test-log-err".to_string());
    let server = McpServer::bind(0, cmd_tx, default_session).await?;
    ...
    // tools/call with neither entries nor text
    let resp = client
        .post(&base_url)
        .header("Content-Type", "application/json")
        .header("Accept", "application/json, text/event-stream")
    ...
    assert!(
        has_error,
        "expected an error response for missing arguments, got: {parsed}"
    );
